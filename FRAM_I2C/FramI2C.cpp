...
FramI2C::FramI2C(PinName sda, PinName scl, int freq, char slave_adr)
    :
    _i2c_p(new I2C(sda, scl)), 
    _i2c(*_i2c_p),
    _I2Caddress(slave_adr)
{
    _i2c.frequency(freq);
}
...
FramI2C::FramI2C(PinName sda, PinName scl, int freq, char slave_adr, framPartNumber partNumber): 
    _partNumber(partNumber),
    _i2c_p(new I2C(sda, scl)), 
    _i2c(*_i2c_p),
    _I2Caddress(slave_adr)// Add in I2C address later
{
  _i2c.frequency(freq);
...
void FramI2C::_readMemory(uint32_t address, uint8_t numberOfBytes, uint8_t *bufRead)
{	
		uint16_t framAddr = (uint16_t)address;
                char bufAddr[2] = {framAddr >> 8, framAddr & 0xFF};
                //char bufferRead[8];
               
                //int numberOfBytesRead = (int)numberOfBytes;
		// Address only correct for 64 through 512 kbit devices
		
                //Wire.beginTransmission(framI2CAddress);
                _i2c.lock();
                _i2c.start();
                _i2c.write(MB85RC_I2C_ADDRESS, bufAddr, 2 , 0);
                
                _i2c.start();
                _i2c.read(MB85RC_I2C_ADDRESS, (char*)bufRead, numberOfBytes, 0);
                _i2c.stop();
                _i2c.unlock();

                 /*for (int i =0; i<8; i++){
                  bufRead[i] = bufferRead[i];
                }*/
                /*
...
void FramI2C::_writeMemory(uint32_t address, uint8_t numberOfBytes, uint8_t *bufferWrite)
{

		uint16_t framAddr = (uint16_t)address;
                //uint8_t bufAddr[2] = {framAddr >>8, framAddr & 0xFF};
                char bufWrite[MAX_NUMBER_BYTE_WRITE+2];
                bufWrite[0] = framAddr >>8;
                bufWrite[1] = framAddr & 0xFF;
                for (int i = 0; i < numberOfBytes; i++){
                  bufWrite[i+2] = bufferWrite[i];
                }
		...
		// Address only correct for 64 through 512 kbit devices
                ...
                _i2c.lock();
                _i2c.start();
                _i2c.write(MB85RC_I2C_ADDRESS, bufWrite , numberOfBytes+2, 0);
                _i2c.stop();
                _i2c.unlock();
		/*
                Wire.beginTransmission(framI2CAddress);
...
void Ring_FramArray::initialize()
{
  static uint32_t newcheck=0;
	...
	// {
		...
		// Check for initialization or existance of pointers
		// Rule to check _pointers.pointerCheck = _pointersAddress+_pointers.tail+pointers.head
		newcheck = _pointersAddress + _pointers.tail + _pointers.head;
		if (newcheck==(uint32_t)_pointers.pointerCheck)
                  //if (newcheck!=216)
                    //if (_pointers.pointerCheck!= (_pointersAddress + _pointers.tail + _pointers.head))
		{
			// Not first run so initialize from FRAM
                        _tailAddress = 0;
			_headAddress = 0;
